...
typedef struct { /*! custom type structure for storing the vertex values*/ float x, y, z; }vertex; 
...
int e = 1; /*! integer that stores the number of faces finalized*/
edge p_edge[400]; /*! the list of all the possible edges*/

int NOVF, NOVT, NOVS, NOEF, NOET, NOES; /*! integer values for maintaining the number of vertices and edges in all the views*/
...
{
	/*! Front_Edge -> Top_Edge -> Side_Vertex */
...
{
	/*! Front_Edge -> Side_Edge -> Top_Vertex */
	int i, j;
	edge temp;
...
void function_7(vertex fv, edge te[], edge se[], edge p_edge[]) {
	/*! Front_Vertex -> Side_Edge -> Top_Edge */
	int i, j;
	edge temp;
...
void wire()
{
	/*! reconstructs the wire frame from user's inpt*/
...
void init(void)
{
	/*! this function innititatse the dispaly and resets the screen to prepare for the same */

}

void DrawCube(void)
{
	/*!the function draws the shape from the possible edges*/
...
void animation(void)
{
	/*! this animates the formed shape so that it can be viewed from every angle*/
...
{
	/*! this function reshapes the shape formed to fit the screen*/
}